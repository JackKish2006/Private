...
//user gives muzzle launch angle 
    ...
    double h_max = (muzzle_velocity * muzzle_velocity * sin(alpha) * sin(alpha)) / (2 * g);
    //Flight time equation (2* Muzzle velocity*(sin(alpha)/g))
    double flight_time = ( 2 * muzzle_velocity * sin(alpha)) / g;
    //distance traveled given by (V^2*sin(2*alpha)/g)
    double x_distance = (((muzzle_velocity * muzzle_velocity) * sin(2 * alpha)) / g);
    // the max range is also given by the same equation as the distance traveled but at an optimal launch angle of 45 degrees
    double max_range = ((muzzle_velocity * muzzle_velocity) * sin(2 * (45 * M_PI / 180 * alpha)) / g);
